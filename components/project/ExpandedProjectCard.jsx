...
const MediaItem = ({ media, index, projectTitle }) => {
  const { ref, inView } = useInView({ triggerOnce: true, threshold: 0.1 });

  if (!inView) {
    return (
      <div ref={ref} className="w-full h-[400px] bg-gray-800 rounded-lg animate-pulse" />
    );
  }
...
const MediaContent = ({ project }) => {
  const allMedia = [
    ...
    ...(project.projVideosUrls
      ? project.projVideosUrls.map((vid) => (typeof vid === "string" ? { priority: 1, url: vid, type: "video" } : { ...vid, type: "video" }))
      : []),
...
const ExpandedProjectCard = ({ project, toolsMap, closeExpanded }) => {
  const techStack = project.projTechStack
    .map((techId, idx) => {
  ...
  const categories = Array.isArray(project.projCategory)
    ...
    : (
      <span className="bg-indigo-600/40 px-3 py-1 text-sm text-white rounded-full border border-indigo-400/30" style={{ boxShadow: "0 0 8px rgba(99, 102, 241, 0.3)" }}>
        {project.projCategory}
      </span>
    );

  return (
    <div className="w-full bg-black/90 backdrop-blur-sm border border-white/20 rounded-3xl text-white py-8 px-4 shadow-2xl" style={{ position: "relative", top: 0, left: 0, zIndex: 100 }}>
      <header className="mb-8 border-b border-white/20 pb-6 w-full grid content-center justify-items-center xl:flex xl:items-center xl:justify-between px-3">
        <article className="w-full grid content-between gap-3 justify-items-start">
          <h1 className="text-3xl font-bold w-full bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">{project.projTitle}</h1>
          ...
          <div className="flex w-max h-16 gap-4 p-3 border border-white/20 rounded-2xl bg-white/5 backdrop-blur-sm">
            {project.projLiveDemoLink && (
              <div className="bigLinksDiv grid content-center justify-items-center hover:scale-110 transition-all duration-300 group">
                ...
                <a href={project.projLiveDemoLink} target="_blank" rel="noopener noreferrer" className="bg-indigo-600 w-10 h-10 flex items-center justify-center p-1 rounded-full hover:bg-indigo-700 transition-colors duration-300 shadow-lg">
            ...
            {project.projSourceCodeLink && (
              <div className="bigLinksDiv grid content-center justify-items-center hover:scale-110 transition-all duration-300 group">
                ...
                <a href={project.projSourceCodeLink} target="_blank" rel="noopener noreferrer" className="bg-white w-10 h-10 flex items-center justify-center p-1 rounded-full hover:bg-gray-200 transition-colors duration-300 shadow-lg">
                  <i className="fas fa-link text-gray-800"></i>
                </a>
              </div>
            )}
            <button onClick={closeExpanded} className="bg-white text-gray-900 hover:bg-gray-200 w-max px-6 rounded-2xl cursor-pointer text-sm font-medium py-2 hover:px-8 transition-all duration-300 shadow-lg border border-white/30">
              See Less
            </button>
          </div>
        </article>
