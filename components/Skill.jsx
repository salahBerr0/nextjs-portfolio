...
export default function Skill() {
  ...
  // Memoize the tool and skill render functions
  const renderTools = useCallback((tool, index) => (
    <article 
      key={`${tool._id}-${index}`} 
      className="inline-flex hover:opacity-100 cursor-auto flex-col items-center justify-center flex-shrink-0"
    >
      <div 
        role="img" 
        aria-label={`${tool.toolName} image`}
  ...
  const renderSkills = useCallback((skill, index) => (
    <article 
      key={`${skill._id}-${index}`} 
      className="inline-flex skillDiv cursor-auto flex-col items-center justify-center flex-shrink-0 group"
    >
      <div 
        role="img" 
        aria-label={`${skill.skillName} image`}
    ...
    <main id='skill' className="grid content-center justify-items-center w-full overflow-hidden px-4 sm:px-8 md:px-16 py-1">
      ...
      >
        ...
        <article className="overflow-hidden whitespace-nowrap w-full py-3">
          <div 
            className="inline-flex animate-scroll gap-4 bigSkillDiv"
            style={{
              animationDuration: `${Skills.length * 2}s`
            }}
          >
            {duplicatedSkills.map(renderSkills)}
          </div>
        </article>
        ...
        <article className="overflow-hidden whitespace-nowrap w-full py-3 border-t border-white/30">
          <div 
            className="inline-flex animate-scroll-reverse gap-4"
            style={{
              animationDuration: `${Tools.length * 1.5}s`
            }}
          >
            {duplicatedTools.map(renderTools)}
          </div>
        </article>
      </section>
    </main>
